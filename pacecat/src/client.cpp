...
int main(int argc, char **argv)
{
  ...
  if (argc != 2)
  {                                                                                  
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "usage: ros2 run pacecat pacecat_client stop/start");
    return 1;
  }
  std::shared_ptr<rclcpp::Node> node = rclcpp::Node::make_shared("pacecat_node_client"); 
  char cmd[8] = {0};
  strcpy(cmd, argv[1]);
