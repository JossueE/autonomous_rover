...
bool ProfileInit(std::shared_ptr<rclcpp::Node> node, ArgData &argdata)
{
	...
	RCLCPP_INFO(node->get_logger(), "ProfileInit: number=%d type=%s frame_id=%s", argdata.num, argdata.type.c_str(), argdata.frame_id.c_str());
	// dual lidar arg
	for (int i = 0; i < argdata.num; i++)
	{
		...
		if (i == 0)
		{
			...
			else if (argdata.type == "uart")
			{
				...
				RCLCPP_INFO(node->get_logger(), "UART config: port=%s baud_rate=%d", arg.arg1.c_str(), arg.arg2);
			}
			else
			{
				RCLCPP_ERROR(node->get_logger(), "Unsupported profile type '%s'", argdata.type.c_str());
				return false;
			}
...
int main(int argc, char *argv[])
{
	...
	RCLCPP_INFO(logger, "Starting bluesea_node");
	RCLCPP_INFO(logger, "ROS2 VERSION: %s", ROS2Verision);
	...
	ArgData argdata;
	if (!ProfileInit(node, argdata))
	{
		RCLCPP_ERROR(logger, "Failed to initialize node profile parameters.");
		return 1;
	}

	if (argdata.connectargs.empty())
	{
		RCLCPP_ERROR(logger, "No connection arguments loaded; check your params file.");
		return 1;
	}
