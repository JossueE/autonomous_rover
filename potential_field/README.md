...
> [!NOTE]
> This repo is based in work from timothee.freville@mil.be

ROS2 Package for differential robots to execute local path planning using the potential field methods.
This package is an excuse to learn C++ and ROS2.
...
### 4. Start Potential Field

```bash
ros2 run potential_field potential_field_node --ros-args \
  -p use_initial_goal:=true \
  -p initial_goal_x:=2.0 \
  -p initial_goal_y:=1.0

```

### 5. Publish PointStamped 
```bash
ros2 topic pub /goal_pose geometry_msgs/msg/PoseStamped "{header: {frame_id: 'odom'}, pose: {position: {x: 2.0, y: 1.0, z: 0.0}, orientation: {w: 1.0}}}"
```
