...
Grid_map::Grid_map(const nav_msgs::msg::OccupancyGrid &map_data)
{
    ...
    cv::Mat obstacle_map(map_.getSize()(0), map_.getSize()(1), CV_8UC1, cv::Scalar(255));
    ...
    for (int y = 0; y < map_.getSize()(1); ++y)
    {
        for (int x = 0; x < map_.getSize()(0); ++x)
        {
            if (*mapDataIter > free_thres_ || *mapDataIter < 0)
            {
                ...
                obstacle_map.at<uchar>(x, y) = 0;
            }
            else
            {
                obstacleData(x, y) = 1.0;
                obstacle_map.at<uchar>(x, y) = 255;
            }
            ++mapDataIter;
        }
    }
    ...
    cv::Mat distance_cv;
    cv::distanceTransform(obstacle_map, distance_cv, cv::DIST_L2, cv::DIST_MASK_PRECISE);
...
std::tuple<int, int> Grid_map::toCellID(const State& state_)
{
...
bool Grid_map::isSingleStateInCollision(const State &current)
{
    // auto init_time = std::chrono::system_clock::now();

    // Get the vehicle footprint as circles in global coordinates
    vehicle_footprint_.getCircles(current, footprint_buffer_);

    // Loop through each circle in the footprint
    for (const auto &circle_itr : footprint_buffer_)
    {
...
bool Grid_map::isSingleStateInCollisionImproved(const State &current)
{
    ...
    if (isInside(pos))
    {
        ...
        if (clearance < bounding_circle.r)
        {
            ...
            return isSingleStateInCollision(current);
        }
        else
        {
