...
Eigen::Affine3f transformToEigen(const geometry_msgs::msg::Transform &transform)
{
    Eigen::Affine3f affine = Eigen::Affine3f::Identity();
    affine.translation() << static_cast<float>(transform.translation.x),
...
pointcloud_clustering_node::pointcloud_clustering_node(/* args */)
      ...
      tf2_listener_(tf2_buffer_)
{
    ...
    RCLCPP_INFO(this->get_logger(), "\033[1;34m----> FRAME_ID: %s\033[0m", FRAME_ID.c_str());
...
void pointcloud_clustering_node::pointCloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
    ...
    if (msg->header.frame_id.empty())
    {
        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                             "Skipping pointcloud: empty source frame.");
        return;
    }

    if (!FRAME_ID.empty() && msg->header.frame_id != FRAME_ID)
    {
        try
        {
            ...
            pcl::transformPointCloud(*input_cloud, *transformed_cloud,
                                     transformToEigen(transform_stamped.transform));
            input_cloud = transformed_cloud;
        }
        catch (const tf2::TransformException &ex)
        {
            RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                                 "Skipping pointcloud: cannot transform from %s to %s: %s",
                                 msg->header.frame_id.c_str(), FRAME_ID.c_str(), ex.what());
            return;
        }
    }
