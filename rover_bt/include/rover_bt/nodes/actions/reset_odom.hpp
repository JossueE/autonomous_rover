...
// One-shot active odometry-recovery routine, run while the rover is held in an
// odom-loss e-stop (blackboard flag emergency_from_odom). RTAB-Map's visual/ICP
// odometry can stay "lost" indefinitely, so passively waiting is not enough.
// delay_sec after the loss began, this node calls /rtabmap/reset_odom ONCE,
// which forces the odometry layer to drop its lost track and re-initialise from
// the current frame.
//
